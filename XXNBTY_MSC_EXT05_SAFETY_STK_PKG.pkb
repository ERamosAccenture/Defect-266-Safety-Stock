...
6-Apr-2015		145			Erwin Ramos				Updated the column period_start_date. It will decode all previous date into the next Monday
2-Jun-2015					Albert Flores			Performance Issue
20-Jun-2015					Erwin Ramos				Added the base_deleted_flag due inconsistency loading. 
*/
--------------------------------------------------------------------------------------------
...
IS
  ...
  IS
    ...
    CURSOR c_msc_plan
    IS 
	--Start 6/2/2015 Albert Flores
	/*SELECT  org
              ,item
              ,period_start_date
         FROM xxnbty_msc_safety_stocks_st
         WHERE org NOT IN (SELECT organization_code
                            FROM msc_trading_partners		--6/2/2015 Albert Flores
	...
	SELECT a.org 
		  ,a.item 
		  ,a.period_start_date 
	FROM xxnbty_msc_safety_stocks_st a 
	WHERE NOT EXISTS (SELECT 1 
					  FROM msc_trading_partners b 
					  WHERE b.partner_type = 3 
					  AND b.organization_code = a.org) 
					  AND process_flag IS NULL;
	--End 6/2/2015 Albert Flores  
  ...
  BEGIN
    ...
    UPDATE xxnbty_msc_safety_stocks_st 
      SET  last_update_date  = SYSDATE 
		  ...
		  ,period_start_date = decode (SIGN (period_start_date - sysdate), -1, next_day(sysdate,'MONDAY'), period_start_date) -- 06-Apr-2015: ERR: Added this line to resolved defect#145, it will decode the previous period_start_date to the next Monday  
    ...
    LOOP
      ...
      FETCH c_required BULK COLLECT INTO g_stg LIMIT gc_limit; --6/2/2015 Albert Flores
      /*
      ...
      END LOOP;
      */
	  --Add Start 6/2/2015 Albert Flores
	  FORALL i IN 1..g_stg.COUNT 
		UPDATE xxnbty_msc_safety_stocks_st 
			SET process_flag = '3' 
			   ,error_description = 'Missing value for required column(i).' 
			WHERE item = g_stg(i).item 
			AND org = g_stg(i).org 
			AND period_start_date = g_stg(i).period_start_date;
	  --End 6/2/2015 Albert Flores
    ...
    FND_CONCURRENT.AF_COMMIT;
    /* Changes Start --6/2/2015 Albert Flores
    ...
    FND_CONCURRENT.AF_COMMIT;
    */ 
	--Changes End 6/2/2015 Albert Flores
    ...
    LOOP
      ...
      FETCH c_user_defined BULK COLLECT INTO g_stg LIMIT gc_limit; --6/2/2015 Albert Flores
	  --Start --6/2/2015 Albert Flores
      /*
      FOR i IN 1..g_stg.COUNT
      LOOP
    ...
    LOOP
      ...
      FETCH c_msc_plan BULK COLLECT INTO g_stg LIMIT gc_limit; --6/2/2015 Albert Flores
      --Start --6/2/2015 Albert Flores
	  /*
      FOR i IN 1..g_stg.COUNT
      LOOP
  ...
  BEGIN
    ...
    LOOP
      ...
      FETCH c_msc_stocks BULK COLLECT INTO g_stocks LIMIT gc_limit; --6/2/2015 Albert Flores
   
      IF g_stocks.COUNT >= 1 AND NOT base_deleted_flag THEN --Added Checking of base_deleted_flag 6/2/2015 Albert Flores
    ...
    LOOP
      ...
      FETCH c_stg BULK COLLECT INTO g_stg LIMIT gc_limit; --6/2/2015 Albert Flores
  ...
  --    WHERE org IN (g_stg(i).org)  
    --    AND item IN (g_stg(i).item)
	  ...
	  FND_CONCURRENT.AF_COMMIT; --6/2/2015 Albert Flores
    END LOOP;
    CLOSE c_stg;
    ...
    LOOP
      ...
      FETCH c_stg_valid BULK COLLECT INTO g_sfty LIMIT gc_limit; --6/2/2015 Albert Flores
   
      IF g_sfty.COUNT >= 1 AND NOT base_deleted_flag THEN 
        --delete all records from base table.
        EXECUTE IMMEDIATE 'DELETE FROM msc_safety_stocks WHERE plan_id = -1'; 
		base_deleted_flag := TRUE; --- 20-JUNE-2015: ERR: Added the base_deleted_flag due inconsistency loading. 
      END IF;
